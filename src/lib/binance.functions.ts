...
export const executeDirectOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      throw new Error(`Binance HTTP ${response.status} WAF Block / Non-JSON Response received.`);
    }

    if (!response.ok) {
      throw new Error(`Binance Rejection [HTTP ${response.status}]: ${JSON.stringify(jsonResult)}`);
    }
