 ...
 * 
 * Strategy Framework Matrix:
 *   - Entry Mechanism: 10-bps Delayed Entry Trap (Bypasses crowd breakout noise)
 *   - Inversion Filter: Freedom Zone Hold Window (-9.9 bps to +24.0 bps)
 *   - Risk Management: Strict -10.0 bps Asymmetric Loss Ceiling Floor from fill
 *   - Target Execution: 12.0 bps Momentum Ratchet Lock & 24.0 bps Macro Target
 *
 * Contract:
...
// --- CORE PIPELINE CONTEXT ASSIGNMENT ---
...
export function runPipeline(inputs: PipelineInputs): ExtendedGateReport {
  ...
  if (inputs.twin?.last) {
    ...
    const stateResult = localPipelineStateMachine.evaluateTick(
      currentPrice, timestamp, verdict, agreement, 60, 20
    );
    engineAction = stateResult.action;

    // Hard emergency cutoff override if drawdown veto hits while a position is live
