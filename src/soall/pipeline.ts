...
class EmbeddedTradeStateMachine {
  private context: PositionContext = {
    state: "FLAT", side: null, signalPrice: 0, entryPrice: 0, size: 1.0, targetPrice: 0, stopPrice: 0, openedAt: 0,
    fusionSnapshot: { verdict: "SILENT", agreement: 0 }, ALI3N: "ACTIVE"
  };
  ...
  getState(): PositionContext { return { ...this.context }; }
  getLedger(): readonly TradeCycleRecord[] { return this.ledger; }

  evaluateTick(currentPrice: number, timestamp: number, verdict: string, agreement: number, targetBps = 60, stopBps = 20): { action: "NONE" | "OPEN" | "CLOSE"; record?: TradeCycleRecord } {
    ...
    if (this.context.state === "ARMED") {
      ...
      this.context = {
        ...this.context, state: "TRAPPING", side, signalPrice: currentPrice, entryPrice: 0, openedAt: timestamp, fusionSnapshot: { verdict, agreement }
      };
      return { action: "NONE" };
    }
    ...
    if (this.context.state === "TRAPPING") {
      ...
      if (currentDriftBps >= this.TRAP_BUFFER_BPS) {
        ...
        this.context = {
          ...this.context, state: "OPEN", entryPrice: currentPrice,
          targetPrice: currentPrice * (1 + orderMultiplier * (targetBps / 10000)),
          stopPrice: currentPrice * (1 - orderMultiplier * (stopBps / 10000)), openedAt: timestamp
        };
        return { action: "OPEN" };
      }
      return { action: "NONE" };
    ...
    if (this.context.state === "OPEN" || this.context.state === "MANAGING" || this.context.state === "HOLDING_STRETCH") {
      ...
      if (currentPnLBps <= this.MAX_LOSS_BPS) exitReason = "HARD_ASYMMETRIC_LOSS_FLOOR";
      ...
      if (exitReason && exitReason.startsWith("GATE_INVERSION")) {
        ...
        if (currentPnLBps <= 0 && currentPnLBps > this.MAX_LOSS_BPS) exitReason = `TRAP_INVERSION_RISK_EXIT`;
      }
      ...
      if (exitReason) {
        ...
        if (exitReason === "HARD_ASYMMETRIC_LOSS_FLOOR") finalReason = `MAX_LOSS_FLOOR_CUT (${currentPnLBps.toFixed(2)}bps)`;
        else if (exitReason === "TARGET_HIT" || currentPnLBps >= this.TARGET_WIN_BPS) finalReason = `TARGET_24BPS_SECURED (${currentPnLBps.toFixed(2)}bps)`;
        else if (exitReason === "STOP_HIT" && currentPnLBps >= this.TIPPING_POINT_BPS) finalReason = `TIPPING_POINT_DIP_LOCKED (${currentPnLBps.toFixed(2)}bps)`;

        const record: TradeCycleRecord = {
          id: `cycle-${Date.now()}`, side, signalPrice: this.context.signalPrice, entryPrice: entry, exitPrice: currentPrice, pnlBps: currentPnLBps,
          reason: finalReason, openedAt: this.context.openedAt, closedAt: timestamp, ALI3N: "SETTLED"
        };

        this.ledger.push(record);
        this.context = {
          state: "FLAT", side: null, signalPrice: 0, entryPrice: 0, size: 1.0, targetPrice: 0, stopPrice: 0, openedAt: 0,
          fusionSnapshot: { verdict, agreement }, ALI3N: "ACTIVE"
        };
...
// Instantiate the isolated engine pipeline instance locally
...
export function runPipeline(
  ...
  // Context updates fetched smoothly from local state cache parameters
